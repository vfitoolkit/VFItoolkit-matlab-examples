function F=ImrohorogluImrohorogluJoines1995_EpsteinZin_ReturnFn(kprime,k,z,r,tau_u, tau_s,h,zeta, epsilon_j,I_j,alpha,delta, A,SSdivw, Tr_beq,MedicalShock,workinglifeincome,g,agej,LumpSum) %i
% Difference with Epstein-Zin is that no longer do the risk aversion inside
% the return function. The only change is line 39, which now just uses F=c
% instead of F=(c.^(1-gamma))/(1-gamma) (and removing gamma from inputs).
