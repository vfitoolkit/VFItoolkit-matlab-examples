...
% But adding Epstein-Zin preferences.
...
% 1. Use vfoptions to state that you are using Epstein-Zin preferences.
vfoptions.exoticpreferences='EpsteinZin'; %Use Epstein-Zin preferences when equal 2 (to turn them off set to zero) 
...
if strcmp(vfoptions.exoticpreferences,'EpsteinZin')
    Params.gamma=3.5; % Risk aversion (Note that this gamma has a slightly different interpretation to that in IIJ1995)
    Params.psi=1.5; % Elasticity of intertemporal substitution
else
    Params.gamma=2; % IIJ1995, pg 91: "1/gamma=0.5"
end
% Comment on gamma: in IIJ1995 there are standard vonNeumannMorgenstern expected utility preferences. 
% So in IIJ1995 gamma determines both the risk aversion and the elasticity of intertemporal
% substitution. With Epstein-Zin preferences these become seperate parameters gamma and psi, respectively.
% For this reason different values of gamma are used with and without Epstein-Zin preferences.
...
if strcmp(vfoptions.exoticpreferences,'EpsteinZin')
    ...
    % Also, now use ImrohorogluImrohorogluJoines1995_EpsteinZin_ReturnFn as return fn.
    ReturnFn=@(aprime,a,z,r,tau_u, tau_s,h,zeta, epsilon_j,I_j,alpha,delta, A,SSdivw, Tr_beq, MedicalShock,workinglifeincome,g,agej,LumpSum) ImrohorogluImrohorogluJoines1995_EpsteinZin_ReturnFn(aprime,a,z,r,tau_u, tau_s,h,zeta, epsilon_j,I_j,alpha,delta, A,SSdivw, Tr_beq, MedicalShock,workinglifeincome,g,agej,LumpSum)
    ReturnFnParamNames={'r','tau_u', 'tau_s', 'h','zeta', 'epsilon_j','I_j','alpha','delta', 'A','SSdivw', 'Tr_beq', 'MedicalShock','workinglifeincome','g','agej','LumpSum'}; %It is important that these are in same order as they appear in 'ImrohorogluImrohogluJoines1995_ReturnFn'
else
...
% Params.gamma=2; % Now done above as part of Epstein-Zin preferences
...
% The following three commented out lines were my original attempt, below that are a
...
% epsilon_j(25:34)=1.39; epsilon_j(35:44)=1.33; epsilon_j(45:65)=0.89; 
% I1989 pg 316 refers to 'interpolated to inbetween years', given that
% Hansen (1993) provides numbers for all years in the form of 'buckets' it
% is unclear exactly what this means. My own attempt (commented out above)
% therefore did not interpolate. From the numbers for epsilon_j that I
% received from Selahattin Imrohogolu shown below it is clear that interpolation was done.
...
% IIJ1995 bans retirees from working. One way to do this is just to
...
% Note that with my original version I_j was needed, but is redundant using
% the original epsilon_j numbers from IIJ1995 as these are anyway zero during retirement.
...
% dividing it by \Pi_{i=1}^{t} (1+n_{i}) (product of all growth rates up till current period)).
...
% Because of use of Epstein-Zin preferences the discount factors and return
% function have already been done.
...
%% Test
simoptions=struct(); % Just use default options
disp('Test StationaryDist')
StationaryDist=StationaryDist_FHorz_Case1(jequaloneDist,AgeWeightsParamNames,Policy,n_d,n_a,n_z,N_j,pi_z,Params,simoptions);
disp('Test StationaryDist: Done')
...
K=AggVars0(1);
N=AggVars0(2);
Q0=Params0.A*(K^(Params0.alpha))*(N^(1-Params0.alpha)); % Cobb-Douglas production fn
