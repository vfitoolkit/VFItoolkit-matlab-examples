...
% 2. Add the relevant parameters to Params (gamma_c, gamma_l, chi, theta; also set h=1, effectively removing it)
...
Params.gdiscount=(1+Params.g)^(1-Params.gamma_c);
...
Params.r=0.021; % interest rate on assets
Params.tau_u=0.001; % set to balance unemployment benefits expenditure
Params.tau_s=0.104; % set to balance social security benefits expenditure
Params.Tr_beq=0.07; % Accidental bequests (IIJ1995 calls this T)
Params.w=2.02;
% The first time I ran this I just used the same initial guesses as for
% exogenous labor, have since changed them to be nearer the actual solution.
...
ReturnFn=@(l,kprime,k,z,r,w,tau_u, tau_s,gamma_c,gamma_l,chi, h,zeta, epsilon_j,I_j,SSdivw, Tr_beq,MedicalShock,workinglifeincome,g,agej,LumpSum, theta,nonSeperableUtility) ImrohorogluImrohorogluJoines1995_EndoLabor_ReturnFn(l,kprime,k,z,r,w,tau_u, tau_s,gamma_c,gamma_l,chi, h,zeta, epsilon_j,I_j,SSdivw, Tr_beq,MedicalShock,workinglifeincome,g,agej,LumpSum, theta,nonSeperableUtility)
ReturnFnParamNames={'r','w','tau_u', 'tau_s','gamma_c','gamma_l','chi', 'h','zeta', 'epsilon_j','I_j','SSdivw', 'Tr_beq','MedicalShock','workinglifeincome','g','agej','LumpSum', 'theta','nonSeperableUtility'}; %It is important that these are in same order as they appear in 'ImrohorogluImrohogluJoines1995_EndoLabor_ReturnFn'
...
FnsToEvaluateParamNames2(5).Names={'r','w','tau_u', 'tau_s','gamma_c','gamma_l','chi', 'h','zeta', 'epsilon_j','I_j','SSdivw', 'Tr_beq','MedicalShock','workinglifeincome','g','agej','LumpSum','theta','nonSeperableUtility'};
FnsToEvaluate_Utility = @(d_val,aprime_val,a_val,z_val,r,w,tau_u, tau_s,gamma_c,gamma_l,chi, h,zeta, epsilon_j,I_j,SSdivw, Tr_beq,MedicalShock,workinglifeincome,g,agej,LumpSum, theta,nonSeperableUtility) ImrohorogluImrohorogluJoines1995_EndoLabor_ReturnFn(d_val,aprime_val,a_val,z_val,r,w,tau_u, tau_s,gamma_c,gamma_l,chi, h,zeta, epsilon_j,I_j,SSdivw, Tr_beq,MedicalShock,workinglifeincome,g,agej,LumpSum, theta,nonSeperableUtility); % Utility
